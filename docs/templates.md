...
$ clconf getv / --template-string '{{div 3 2}}'
1
```
...
$ MYENV= clconf getv / --template-string '[{{getenv "MYENV"}}]'
[]

# Default used when defined and empty
$ MYENV= clconf getv / --template-string '[{{getenv "MYENV" "adefault"}}]'
[adefault]

$ clconf getv / --template-string '[{{getenv "MYENV" "adefault"}}]'
[adefault]

# Does not fail if not defined
$ clconf getv / --template-string '[{{getenv "MYENV"}}]'
[]
```
...
$ clconf --pipe getv --template-string '{{getksvs "/foo/dog"}}' <<EOF
foo:
  dog: woof
  bird: tweet
  cat: meow
...
$ clconf --pipe getv --template-string '{{getsvs "/foo/dog"}}' <<EOF
foo:
  dog: woof
  bird: tweet
  cat: meow
...
$ clconf --pipe getv / --template-string '{{range gets "/foo/bar"}}k: {{.Key}}, v: {{.Value}}{{"\n"}}{{end}}' <<EOF
foo:
  bar: bip
  zip: zap
EOF
...
# With default value
$ clconf --pipe getv / --template-string '{{getv "/foo/hip" "hop"}}' <<EOF
foo:
  bar: bip
  zip: zap
EOF
...
# Missing, no default
$ clconf --pipe getv / --template-string '{{getv "/foo/hip"}}' <<EOF
foo:
  bar: bip
  zip: zap
EOF
...
$ clconf --pipe getv / --template-string '{{getvs "/bar/*"}}' <<EOF
foo:
  buzz: 1 
  zap: 10
  hop: 2
...
clconf --pipe getv / --template-string '{{getvs "/foo/buzz"}}' <<EOF
foo:
  buzz: 1 
  zap: 10
  hop: 2
...
$ clconf --pipe getv / --template-string '{{getvs "/*/*"}}' <<EOF
foo:
  buzz: 1 
  zap: 10
  hop: 2
...
```text
$ clconf getv / --template-string '{{lookupIP "localhost"}}'
[127.0.0.1]
```
...
$ clconf --pipe getv / --template-string '[{{join (ls "/foo/hip") ","}}]' <<EOF
foo:
  bar: bip
  hip:
    hop: hoop
...
$ clconf --pipe getv / --template-string '[{{join (ls "/") ","}}]' <<EOF
foo:
  bar: bip
  hip:
    hop: hoop
...
$ clconf --pipe getv / --template-string '[{{join (ls "/bar") ","}}]' <<EOF
foo:
  bar: bip
  hip:
    hop: hoop
...
$ clconf --pipe getv / --template-string '[{{join (lsdir "/foo/hip") ","}}]' <<EOF
foo:
  bar: baz
  hip:
  - hop: hap
...
$ clconf --pipe getv / --template-string '[{{join (lsdir "/foo/zip") ","}}]' <<EOF
foo:
  bar: baz
  hip:
  - hop: hap
...
```text
$ clconf getv / --template-string '{{mod 10 3}}'
1
```
...
$ clconf getv / --template-string '{{parseBool "T"}}'
true

$ clconf getv / --template-string '{{parseBool "F"}}'
false

$ clconf getv / --template-string '{{parseBool "1"}}'
true

$ clconf getv / --template-string '{{parseBool "R"}}'
...
$ clconf getv / --template-string '{{parseBool "0"}}'
false
```
